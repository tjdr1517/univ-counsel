...
export async function POST(request: Request) {
  ...
  const row = await runtimeEnv().DB.prepare("SELECT * FROM users WHERE email = ? LIMIT 1").bind(email).first<Record<string, unknown>>();
  if (!row) return jsonError("이메일 또는 비밀번호가 올바르지 않습니다.", 401);
  const candidate = await hashPassword(password, String(row.password_salt));
  if (!timingSafeEqual(candidate.hash, String(row.password_hash))) return jsonError("이메일 또는 비밀번호가 올바르지 않습니다.", 401);
