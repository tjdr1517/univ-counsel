...
type SlotRow = { id: string; teacher_id: string; student_id: string | null; date: string; time: string };
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const slot = await db.prepare("SELECT id,teacher_id,student_id,date,time FROM appointment_slots WHERE id=? LIMIT 1").bind(id).first<SlotRow>();
  ...
  if (action === "reserve") {
    ...
    const result = await db.prepare("UPDATE appointment_slots SET student_id=?,reserved_at=CURRENT_TIMESTAMP WHERE id=? AND student_id IS NULL").bind(auth.user.id, id).run();
  ...
  if (action === "cancel") {
    ...
    await db.prepare("UPDATE appointment_slots SET student_id=NULL,reserved_at=NULL WHERE id=?").bind(id).run();
    return Response.json({ ok: true });
  }
