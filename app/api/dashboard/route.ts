...
export async function GET(request: Request) {
  ...
  const interests: InterestUniversity[] = interestResult.results.map((row: Record<string, unknown>) => ({
    id: String(row.id), teacherId: String(row.teacher_id), studentId: String(row.student_id), studentName: String(row.student_name),
    university: String(row.university), department: String(row.department), track: String(row.track), admissionName: String(row.admission_name ?? ""), schoolRecommendation: Boolean(row.school_recommendation), evaluationFactors: String(row.evaluation_factors ?? ""), minimum: String(row.minimum),
