...
import type { Announcement, Attachment, Consultation } from "../../../lib/types";
...
export async function GET(request: Request) {
  ...
  const [consultationResult, announcementResult, attachmentResult] = await Promise.all([
  ...
  const consultations: Consultation[] = consultationResult.results.map((row: Record<string, unknown>) => ({
    ...
    plans: JSON.parse(String(row.plans_json || "[]")), attachments: files.get(String(row.id)) ?? [],
  }));
  ...
  return Response.json({ user, consultations, announcements, students, pendingStudents });
}
