...
function AppointmentModal({ initialDate, onClose, onSaved }: { initialDate: string; onClose: () => void; onSaved: () => void }) {
  const [saving, setSaving] = useState(false); const [error, setError] = useState("");
  const today = new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Seoul" }).format(new Date());
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setSaving(true); setError(""); const form = new FormData(event.currentTarget); try { await api("/api/appointments", { method: "POST", body: JSON.stringify({ date: form.get("date"), time: form.get("time") }) }); onSaved(); } catch (reason) { setError(reason instanceof Error ? reason.message : "상담 시간을 추가하지 못했습니다."); setSaving(false); } };
  return <Modal title="상담 가능 시간 추가" onClose={onClose}><form className="modal-form" onSubmit={submit}><div className="field-row"><label>상담 일자<input name="date" type="date" min={today} required defaultValue={initialDate && initialDate >= today ? initialDate : today} /></label><label>시작 시간<input name="time" type="time" required /></label></div><p className="form-help">학생 한 명이 신청할 수 있는 상담 시간 한 칸이 생성됩니다.</p>{error && <p className="form-error">{error}</p>}<div className="modal-actions"><button type="button" className="secondary-button" onClick={onClose}>취소</button><button type="submit" className="primary-button" disabled={saving}>{saving ? "추가 중…" : "상담 시간 추가"}</button></div></form></Modal>;
}
