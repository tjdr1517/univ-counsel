...
import { Bell, CalendarClock, Check, ChevronLeft, ChevronRight, ClipboardList, FileText, GraduationCap, Home, LogOut, Menu, Megaphone, Paperclip, Pencil, Plus, Search, ShieldCheck, Trash2, Upload, Users, X } from "lucide-react";
...
type View = "home" | "records" | "interests" | "appointments" | "students" | "announcements";

const viewHashes: Record<View, string> = { home: "", records: "records", interests: "interests", appointments: "appointments", students: "students", announcements: "announcements" };
  ...
  return <div className="app-shell">
    ...
    <main className="main-panel">
      ...
      <div className="page-wrap">
        ...
        {view === "appointments" && <AppointmentsPage user={user} appointments={data.appointments} onNew={date => { setAppointmentDate(date); setAppointmentModal(true); }} onReserve={async id => { await api(`/api/appointments/${id}`, { method: "PATCH", body: JSON.stringify({ action: "reserve" }) }); notify("상담을 신청했습니다."); }} onCancel={async id => { await api(`/api/appointments/${id}`, { method: "PATCH", body: JSON.stringify({ action: "cancel" }) }); notify("상담 예약을 취소했습니다."); }} onDelete={async id => { await api(`/api/appointments/${id}`, { method: "DELETE" }); notify("상담 시간을 삭제했습니다."); }} />}
...
function AppointmentsPage({ user, appointments, onNew, onReserve, onCancel, onDelete }: { user: User; appointments: AppointmentSlot[]; onNew: (date: string) => void; onReserve: (id: string) => Promise<void>; onCancel: (id: string) => Promise<void>; onDelete: (id: string) => Promise<void> }) {
  const [busy, setBusy] = useState("");
  const [error, setError] = useState("");
  const teacher = user.role === "teacher";
  ...
  const cancel = (slot: AppointmentSlot) => { if (window.confirm(`${formatDate(slot.date)} ${slot.time} 예약을 취소할까요?`)) run(slot.id, () => onCancel(slot.id)); };
  ...
  return <><div className="section-heading"><div><span className="eyebrow">COUNSELING APPOINTMENT</span><h1>{teacher ? "상담 일정 관리" : "상담 신청"}</h1><p>{teacher ? "캘린더에서 날짜를 선택해 상담 시간을 등록하고 예약을 확인합니다." : "캘린더에서 날짜를 고른 뒤 신청 가능한 시간을 선택하세요."}</p></div>{teacher && <button className="primary-button" onClick={() => onNew(selectedDate < today ? today : selectedDate)}><Plus size={17} />선택 날짜에 시간 추가</button>}</div>{error && <p className="form-error appointment-error">{error}</p>}<div className="appointment-calendar-layout"><section className="calendar-card"><header className="calendar-toolbar"><button type="button" onClick={() => moveMonth(-1)} aria-label="이전 달"><ChevronLeft /></button><h2>{year}년 {monthNumber}월</h2><button type="button" onClick={() => moveMonth(1)} aria-label="다음 달"><ChevronRight /></button></header><div className="calendar-weekdays">{["일", "월", "화", "수", "목", "금", "토"].map(day => <span key={day}>{day}</span>)}</div><div className="calendar-grid">{calendarDays.map((date, index) => {
    ...
    return <button type="button" className={`calendar-day ${date === selectedDate ? "selected" : ""} ${date === today ? "today" : ""}`} disabled={disabled} aria-pressed={date === selectedDate} onClick={() => setSelectedDate(date)} key={date}><span>{Number(date.slice(8))}</span><div className="calendar-dots">{slots.slice(0, 3).map(slot => <i className={slot.isMine ? "mine" : slot.status} key={slot.id} />)}</div>{slots.length > 0 && <small>{teacher ? `${slots.length}개` : availableCount ? `가능 ${availableCount}` : "예약됨"}</small>}</button>;
  })}</div><footer className="calendar-legend"><span><i className="available" />신청 가능</span><span><i className="reserved" />예약됨</span>{!teacher && <span><i className="mine" />내 예약</span>}</footer></section><section className="day-schedule"><header><div><span>선택한 날짜</span><h2>{formatDate(selectedDate)}</h2></div>{teacher && <button type="button" onClick={() => onNew(selectedDate < today ? today : selectedDate)}><Plus />시간 추가</button>}</header><div className="time-slot-list">{selectedSlots.map(slot => <article className={`time-slot-row ${slot.isMine ? "mine" : ""}`} key={slot.id}><strong>{slot.time}</strong>{teacher ? <div className="time-slot-copy">{slot.status === "reserved" ? <><span className="status-pill reserved">예약 완료</span><small>{slot.studentNumber ?? "-"}번 · {slot.studentName}</small></> : <span className="status-pill available">신청 가능</span>}</div> : <div className="time-slot-copy">{slot.isMine ? <span className="status-pill mine">내 예약</span> : slot.status === "reserved" ? <span className="status-pill reserved">예약됨</span> : <span className="status-pill available">신청 가능</span>}</div>}<div className="appointment-actions">{teacher ? <>{slot.status === "reserved" && <button type="button" disabled={busy === slot.id} onClick={() => cancel(slot)}>예약 취소</button>}<button type="button" className="danger" disabled={busy === slot.id} onClick={() => remove(slot)}><Trash2 />삭제</button></> : slot.isMine ? <button type="button" disabled={busy === slot.id} onClick={() => cancel(slot)}>취소</button> : slot.status === "available" ? <button type="button" className="reserve-button" disabled={busy === slot.id} onClick={() => run(slot.id, () => onReserve(slot.id))}>{busy === slot.id ? "신청 중…" : "신청"}</button> : <button type="button" disabled>예약됨</button>}</div></article>)}{!selectedSlots.length && <div className="calendar-no-slots"><CalendarClock /><strong>{teacher ? "이 날짜에 등록된 시간이 없습니다." : "신청 가능한 시간이 없습니다."}</strong>{teacher && <button type="button" onClick={() => onNew(selectedDate < today ? today : selectedDate)}>상담 시간 추가</button>}</div>}</div></section></div></>;
}
