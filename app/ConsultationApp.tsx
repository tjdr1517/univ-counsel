...
export default function ConsultationApp() {
  ...
  const interests = useMemo(() => {
    ...
    return keyword ? rows.filter(row => `${row.studentName} ${row.university} ${row.department} ${row.track} ${row.evaluationFactors} ${row.minimum} ${row.memo}`.toLowerCase().includes(keyword)) : rows;
  }, [data, search]);
  ...
  return <div className="app-shell">
    ...
    <main className="main-panel">
      ...
      <div className="page-wrap">
        ...
        {view === "appointments" && <AppointmentsPage user={user} appointments={data.appointments} onNew={date => { setAppointmentDate(date); setAppointmentModal(true); }} onReserve={async id => { await api(`/api/appointments/${id}`, { method: "PATCH", body: JSON.stringify({ action: "reserve" }) }); notify("상담을 신청했습니다. 교사 승인을 기다려 주세요."); }} onApprove={async id => { await api(`/api/appointments/${id}`, { method: "PATCH", body: JSON.stringify({ action: "approve" }) }); notify("상담 예약을 확정했습니다."); }} onCancel={async id => { await api(`/api/appointments/${id}`, { method: "PATCH", body: JSON.stringify({ action: "cancel" }) }); notify("상담 예약을 취소했습니다."); }} onDelete={async id => { await api(`/api/appointments/${id}`, { method: "DELETE" }); notify("상담 시간을 삭제했습니다."); }} />}
        {view === "students" && <StudentsPage data={data} onApprove={async id => { await api(`/api/users/${id}/approve`, { method: "POST" }); notify("학생 계정을 승인했습니다."); }} />}
        {view === "announcements" && <AnnouncementsPage user={user} announcements={data.announcements} onNew={() => { setEditingNotice(null); setNoticeModal(true); }} onEdit={notice => { setEditingNotice(notice); setNoticeModal(true); }} onDelete={async notice => { await api(`/api/announcements/${notice.id}`, { method: "DELETE" }); notify("공지를 삭제했습니다."); }} />}
        {view === "settings" && user.role === "teacher" && <SettingsPage />}
      </div>
    </main>
    ...
    {appointmentModal && <AppointmentModal initialDate={appointmentDate} onClose={() => setAppointmentModal(false)} onSaved={() => { setAppointmentModal(false); notify("상담 가능 시간을 추가했습니다."); }} />}
    {noticeModal && <NoticeModal notice={editingNotice} onClose={() => { setNoticeModal(false); setEditingNotice(null); }} onSaved={attachmentFailed => { const edited = Boolean(editingNotice); setNoticeModal(false); setEditingNotice(null); notify(attachmentFailed ? `${edited ? "공지는 수정했지만" : "공지는 게시했지만"} 첨부 파일은 올리지 못했습니다.` : edited ? "공지를 수정했습니다." : "공지를 게시했습니다."); }} />}
...
function AppointmentsPage({ user, appointments, onNew, onReserve, onApprove, onCancel, onDelete }: { user: User; appointments: AppointmentSlot[]; onNew: (date: string) => void; onReserve: (id: string) => Promise<void>; onApprove: (id: string) => Promise<void>; onCancel: (id: string) => Promise<void>; onDelete: (id: string) => Promise<void> }) {
  ...
  const cancel = (slot: AppointmentSlot, label = "예약") => { if (window.confirm(`${formatDate(slot.date)} ${slot.time} ${label}을 취소할까요?`)) run(slot.id, () => onCancel(slot.id)); };
  ...
  return <><div className="section-heading"><div><span className="eyebrow">COUNSELING APPOINTMENT</span><h1>{teacher ? "상담 일정 관리" : "상담 신청"}</h1><p>{teacher ? "학생의 상담 신청을 확인하고 승인하면 예약이 확정됩니다." : "원하는 시간을 신청하면 교사 승인 후 예약이 확정됩니다."}</p></div>{teacher && <button className="primary-button" onClick={() => onNew(selectedDate < today ? today : selectedDate)}><Plus size={17} />선택 날짜에 시간 추가</button>}</div>{error && <p className="form-error appointment-error">{error}</p>}<div className="appointment-calendar-layout"><section className="calendar-card"><header className="calendar-toolbar"><button type="button" onClick={() => moveMonth(-1)} aria-label="이전 달"><ChevronLeft /></button><h2>{year}년 {monthNumber}월</h2><button type="button" onClick={() => moveMonth(1)} aria-label="다음 달"><ChevronRight /></button></header><div className="calendar-weekdays">{["일", "월", "화", "수", "목", "금", "토"].map(day => <span key={day}>{day}</span>)}</div><div className="calendar-grid">{calendarDays.map((date, index) => {
    if (!date) return <span className="calendar-empty" key={`empty-${index}`} />;
    const slots = upcoming.filter(slot => slot.date === date);
    const availableCount = slots.filter(slot => slot.status === "available").length;
    const pendingCount = slots.filter(slot => slot.bookingStatus === "pending").length;
  ...
  })}</div><footer className="calendar-legend"><span><i className="available" />신청 가능</span><span><i className="reserved" />예약됨</span>{teacher && <span><i className="pending" />승인 대기</span>}{!teacher && <><span><i className="pending" />승인 대기</span><span><i className="confirmed" />예약 확정</span></>}</footer></section><section className="day-schedule"><header><div><span>선택한 날짜</span><h2>{formatDate(selectedDate)}</h2></div>{teacher && <button type="button" onClick={() => onNew(selectedDate < today ? today : selectedDate)}><Plus />시간 추가</button>}</header><div className="time-slot-list">{selectedSlots.map(slot => <article className={`time-slot-row ${slot.isMine ? "mine" : ""} ${slot.bookingStatus}`} key={slot.id}><strong>{slot.time}</strong>{teacher ? <div className="time-slot-copy">{slot.bookingStatus === "pending" ? <><span className="status-pill pending">승인 대기</span><small>{slot.studentNumber ?? "-"}번 · {slot.studentName}</small></> : slot.bookingStatus === "confirmed" ? <><span className="status-pill confirmed">예약 확정</span><small>{slot.studentNumber ?? "-"}번 · {slot.studentName}</small></> : <span className="status-pill available">신청 가능</span>}</div> : <div className="time-slot-copy">{slot.isMine ? <span className={`status-pill ${slot.bookingStatus}`}>{slot.bookingStatus === "pending" ? "승인 대기" : "예약 확정"}</span> : slot.status === "reserved" ? <span className="status-pill reserved">예약됨</span> : <span className="status-pill available">신청 가능</span>}</div>}<div className="appointment-actions">{teacher ? <>{slot.bookingStatus === "pending" && <><button type="button" className="reserve-button" disabled={busy === slot.id} onClick={() => run(slot.id, () => onApprove(slot.id))}>승인</button><button type="button" disabled={busy === slot.id} onClick={() => cancel(slot, "신청")}>거절</button></>}{slot.bookingStatus === "confirmed" && <button type="button" disabled={busy === slot.id} onClick={() => cancel(slot)}>예약 취소</button>}<button type="button" className="danger" disabled={busy === slot.id} onClick={() => remove(slot)}><Trash2 />삭제</button></> : slot.isMine ? <button type="button" disabled={busy === slot.id} onClick={() => cancel(slot, slot.bookingStatus === "pending" ? "신청" : "예약")}>취소</button> : slot.status === "available" ? <button type="button" className="reserve-button" disabled={busy === slot.id} onClick={() => run(slot.id, () => onReserve(slot.id))}>{busy === slot.id ? "신청 중…" : "신청"}</button> : <button type="button" disabled>예약됨</button>}</div></article>)}{!selectedSlots.length && <div className="calendar-no-slots"><CalendarClock /><strong>{teacher ? "이 날짜에 등록된 시간이 없습니다." : "신청 가능한 시간이 없습니다."}</strong>{teacher && <button type="button" onClick={() => onNew(selectedDate < today ? today : selectedDate)}>상담 시간 추가</button>}</div>}</div></section></div></>;
}

type NotificationSettings = { notificationsEnabled: boolean; discordConfigured: boolean; maskedWebhook: string };
...
function InterestModal({ user, students, onClose, onSaved }: { user: User; students: User[]; onClose: () => void; onSaved: () => void }) {
  const [saving, setSaving] = useState(false); const [error, setError] = useState("");
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setSaving(true); setError(""); const form = new FormData(event.currentTarget); try { await api("/api/interests", { method: "POST", body: JSON.stringify({ studentId: form.get("studentId"), university: form.get("university"), department: form.get("department"), track: form.get("track"), evaluationFactors: form.get("evaluationFactors"), minimum: form.get("minimum"), schoolGrade: form.get("schoolGrade"), cutoff2023: form.get("cutoff2023"), cutoff2024: form.get("cutoff2024"), cutoff2025: form.get("cutoff2025"), priority: form.get("priority"), memo: form.get("memo") }) }); onSaved(); } catch (reason) { setError(reason instanceof Error ? reason.message : "저장하지 못했습니다."); setSaving(false); } };
  return <Modal title="관심 대학 추가" onClose={onClose}><form className="modal-form" onSubmit={submit}>{user.role === "teacher" && <label>학생<select name="studentId" required defaultValue=""><option value="" disabled>학생 선택</option>{students.map(student => <option key={student.id} value={student.id}>{student.studentNumber}번 · {student.name}</option>)}</select></label>}<div className="field-row"><label>대학<input name="university" required placeholder="대학명" /></label><label>학과<input name="department" required placeholder="학과·계열" /></label></div><div className="field-row"><label>전형 종류<select name="track" required defaultValue=""><option value="" disabled>전형 선택</option><option>학생부 종합 전형</option><option>학생부 교과 전형</option><option>논술 전형</option><option>실기(특기자) 전형</option><option>기타 전형</option></select></label><label>지망 순위<input name="priority" type="number" min="1" max="99" required defaultValue="1" placeholder="1" /></label></div><label><span className="field-label">평가 요소<span className="help-wrap"><button type="button" className="help-button" aria-label="평가 요소 작성 예시">?</button><span className="help-popover" role="tooltip"><strong>작성 예시</strong><span>생기부 50 + 교과 50</span><span>교과 100</span></span></span></span><input name="evaluationFactors" placeholder="예: 생기부 50 + 교과 50" /></label><div className="field-row"><label>수능 최저<input name="minimum" placeholder="예: 2개 합 5 / 없음" /></label><label>내신 등급<input name="schoolGrade" placeholder="예: 2.3등급" /></label></div><div className="cutoff-grid"><label>2023년 입결<input name="cutoff2023" placeholder="등급·점수" /></label><label>2024년 입결<input name="cutoff2024" placeholder="등급·점수" /></label><label>2025년 입결<input name="cutoff2025" placeholder="등급·점수" /></label></div><label>기타 메모<textarea name="memo" rows={4} placeholder="모집 인원, 전년도 결과, 확인할 내용 등을 기록하세요." /></label>{error && <p className="form-error">{error}</p>}<div className="modal-actions"><button type="button" className="secondary-button" onClick={onClose}>취소</button><button className="primary-button" disabled={saving || (user.role === "teacher" && !students.length)}>{saving ? "저장 중…" : "관심 대학 저장"}</button></div></form></Modal>;
}

function AppointmentModal({ initialDate, onClose, onSaved }: { initialDate: string; onClose: () => void; onSaved: () => void }) {
  const [saving, setSaving] = useState(false); const [error, setError] = useState("");
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!selectedTimes.length) { setError("추가할 시간을 하나 이상 선택해 주세요."); return; } setSaving(true); setError(""); const form = new FormData(event.currentTarget); try { await api("/api/appointments", { method: "POST", body: JSON.stringify({ date: form.get("date"), times: selectedTimes }) }); onSaved(); } catch (reason) { setError(reason instanceof Error ? reason.message : "상담 시간을 추가하지 못했습니다."); setSaving(false); } };
  return <Modal title="상담 가능 시간 추가" onClose={onClose}><form className="modal-form appointment-time-form" onSubmit={submit}><label>상담 일자<input name="date" type="date" min={today} required defaultValue={initialDate && initialDate >= today ? initialDate : today} /></label><fieldset className="time-picker-field"><legend><span>시간 선택</span><small>여러 시간을 한 번에 선택할 수 있어요</small></legend><div className="time-picker-grid">{timeOptions.map(time => { const selected = selectedTimes.includes(time); return <button key={time} type="button" className={selected ? "selected" : ""} aria-pressed={selected} onClick={() => toggleTime(time)}><span>{Number(time.slice(0, 2)) < 12 ? "오전" : "오후"}</span><strong>{time}</strong>{selected && <Check />}</button>; })}</div></fieldset><div className="selected-time-summary"><CalendarClock /><div><strong>{selectedTimes.length ? `${selectedTimes.length}개 시간 선택됨` : "시간을 선택해 주세요"}</strong><span>{selectedTimes.length ? selectedTimes.join(" · ") : "버튼을 다시 누르면 선택이 해제됩니다."}</span></div></div>{error && <p className="form-error">{error}</p>}<div className="modal-actions"><button type="button" className="secondary-button" onClick={onClose}>취소</button><button type="submit" className="primary-button" disabled={saving || !selectedTimes.length}>{saving ? "추가 중…" : selectedTimes.length ? `${selectedTimes.length}개 시간 추가` : "시간 선택"}</button></div></form></Modal>;
}

function NoticeModal({ notice, onClose, onSaved }: { notice: Announcement | null; onClose: () => void; onSaved: (attachmentFailed?: boolean) => void }) {
  const [saving, setSaving] = useState(false); const [error, setError] = useState("");
