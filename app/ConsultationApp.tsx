...
function AppointmentModal({ initialDate, onClose, onSaved }: { initialDate: string; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal title="상담 가능 일정 추가" onClose={onClose}><form className="modal-form appointment-time-form" onSubmit={submit}><label>상담 일자<input name="date" type="date" min={today} required defaultValue={initialDate && initialDate >= today ? initialDate : today} /></label><div className="slot-mode-tabs" role="tablist" aria-label="일정 등록 방식"><button type="button" role="tab" aria-selected={slotMode === "time"} className={slotMode === "time" ? "active" : ""} onClick={() => { setSlotMode("time"); setError(""); }}>시간으로 등록</button><button type="button" role="tab" aria-selected={slotMode === "period"} className={slotMode === "period" ? "active" : ""} onClick={() => { setSlotMode("period"); setError(""); }}>교시로 등록</button></div><fieldset className="time-picker-field"><legend><span>{slotMode === "time" ? "시간 선택" : "교시 선택"}</span><small>여러 개를 한 번에 선택할 수 있어요</small></legend>{slotMode === "time" ? <div className="time-picker-grid">{timeOptions.map(time => { const selected = selectedTimes.includes(time); return <button key={time} type="button" className={selected ? "selected" : ""} aria-pressed={selected} onClick={() => toggleTime(time)}><span>{Number(time.slice(0, 2)) < 12 ? "오전" : "오후"}</span><strong>{time}</strong>{selected && <Check />}</button>; })}</div> : <div className="period-picker-grid">{Array.from({ length: 10 }, (_, index) => index + 1).map(period => { const selected = selectedPeriods.includes(period); return <button key={period} type="button" className={selected ? "selected" : ""} aria-pressed={selected} onClick={() => togglePeriod(period)}><strong>{period}교시</strong>{selected && <Check />}</button>; })}</div>}</fieldset><div className="selected-time-summary"><CalendarClock /><div><strong>{selectedCount ? `${selectedCount}개 ${slotMode === "time" ? "시간" : "교시"} 선택됨` : `${slotMode === "time" ? "시간" : "교시"}를 선택해 주세요`}</strong><span>{selectedCount ? selectedLabels.join(" · ") : "버튼을 다시 누르면 선택이 해제됩니다."}</span></div></div>{error && <p className="form-error">{error}</p>}<div className="modal-actions"><button type="button" className="secondary-button" onClick={onClose}>취소</button><button type="submit" className="primary-button" disabled={saving || !selectedCount}>{saving ? "추가 중…" : selectedCount ? `${selectedCount}개 일정 추가` : `${slotMode === "time" ? "시간" : "교시"} 선택`}</button></div></form></Modal>;
}
