...
function AuthScreen({ onAuthenticated }: { onAuthenticated: (user: User) => void }) {
  ...
  return <main className="auth-page"><section className="auth-card"><div className="auth-brand"><span className="brand-mark">담</span><div><strong>담다</strong><small>대입 상담 기록</small></div></div><h1>{mode === "login" ? "로그인" : "계정 만들기"}</h1><p>{mode === "login" ? "이메일과 비밀번호로 시작하세요." : "학생 계정은 가입 후 교사 승인이 필요합니다."}</p>
    ...
    <form onSubmit={submit} className="auth-form">
      ...
      <label>이메일<input name="email" type="email" required autoComplete="email" placeholder="name@example.com" /></label>
