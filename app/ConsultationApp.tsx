...
import { Bell, CalendarClock, Check, ChevronRight, ClipboardList, FileText, GraduationCap, Home, LogOut, Menu, Megaphone, Paperclip, Pencil, Plus, Search, ShieldCheck, Trash2, Upload, Users, X } from "lucide-react";
  ...
  return <div className="app-shell">
    ...
    <main className="main-panel">
      ...
      <div className="page-wrap">
        ...
        {view === "appointments" && <AppointmentsPage user={user} appointments={data.appointments} onNew={() => setAppointmentModal(true)} onReserve={async id => { await api(`/api/appointments/${id}`, { method: "PATCH", body: JSON.stringify({ action: "reserve" }) }); notify("상담을 신청했습니다."); }} onCancel={async id => { await api(`/api/appointments/${id}`, { method: "PATCH", body: JSON.stringify({ action: "cancel" }) }); notify("상담 예약을 취소했습니다."); }} onDelete={async id => { await api(`/api/appointments/${id}`, { method: "DELETE" }); notify("상담 시간을 삭제했습니다."); }} />}
    ...
    {appointmentModal && <AppointmentModal onClose={() => setAppointmentModal(false)} onSaved={() => { setAppointmentModal(false); notify("상담 가능 시간을 추가했습니다."); }} />}
...
function AppointmentsPage({ user, appointments, onNew, onReserve, onCancel, onDelete }: { user: User; appointments: AppointmentSlot[]; onNew: () => void; onReserve: (id: string) => Promise<void>; onCancel: (id: string) => Promise<void>; onDelete: (id: string) => Promise<void> }) {
  const [busy, setBusy] = useState("");
  const [error, setError] = useState("");
  const teacher = user.role === "teacher";
  ...
  return <><div className="section-heading"><div><span className="eyebrow">COUNSELING APPOINTMENT</span><h1>{teacher ? "상담 가능 시간" : "상담 신청"}</h1><p>{teacher ? "학생이 신청할 수 있는 상담 일자와 시간을 등록합니다." : "신청 가능한 시간을 선택하세요. 다른 학생의 정보는 표시되지 않습니다."}</p></div>{teacher && <button className="primary-button" onClick={onNew}><Plus size={17} />상담 시간 추가</button>}</div>{error && <p className="form-error appointment-error">{error}</p>}<div className="appointment-grid">{upcoming.map(slot => <article className={`appointment-card ${slot.status} ${slot.isMine ? "mine" : ""}`} key={slot.id}><div className="appointment-date"><strong>{formatDate(slot.date)}</strong><span>{slot.time}</span></div>{teacher ? <div className="appointment-status">{slot.status === "reserved" ? <><span className="status-pill reserved">예약 완료</span><strong>{slot.studentNumber ?? "-"}번 · {slot.studentName}</strong></> : <><span className="status-pill available">신청 가능</span><strong>아직 신청한 학생이 없습니다.</strong></>}</div> : <div className="appointment-status">{slot.isMine ? <><span className="status-pill mine">내 예약</span><strong>신청한 상담 시간입니다.</strong></> : slot.status === "reserved" ? <><span className="status-pill reserved">예약됨</span><strong>신청할 수 없는 시간입니다.</strong></> : <><span className="status-pill available">신청 가능</span><strong>지금 신청할 수 있습니다.</strong></>}</div>}<div className="appointment-actions">{teacher ? <>{slot.status === "reserved" && <button type="button" disabled={busy === slot.id} onClick={() => cancel(slot)}>예약 취소</button>}<button type="button" className="danger" disabled={busy === slot.id} onClick={() => remove(slot)}><Trash2 />삭제</button></> : slot.isMine ? <button type="button" disabled={busy === slot.id} onClick={() => cancel(slot)}>예약 취소</button> : slot.status === "available" ? <button type="button" className="reserve-button" disabled={busy === slot.id} onClick={() => run(slot.id, () => onReserve(slot.id))}>{busy === slot.id ? "신청 중…" : "상담 신청"}</button> : <button type="button" disabled>예약됨</button>}</div></article>)}</div>{!upcoming.length && <Empty text={teacher ? "등록된 상담 가능 시간이 없습니다." : user.teacherId ? "신청 가능한 상담 시간이 없습니다." : "담당 교사 연결 후 상담을 신청할 수 있습니다."} />}</>;
}
